...
from scraper_pcs.calculate_scores import (
    calculate_match_points,
    calculate_match_standings,
    calculate_stage_points,
)
...
print(
    f"Value for MAKE_POST: {str(MAKE_POST)} (Upload: {os.getenv('IMGUR_UPLOAD')}, Forum: {os.getenv('FORUM_POST')})"
)
...
def find_matches_to_scrape(results_data: result_objects.StageResults) -> pd.DataFrame:
    ...
    match_not_processed_yet = ~results_data.matches["MATCH"].isin(
        results_data.all_results["MATCH"].unique()
    )
    matches_to_scrape = results_data.matches[
        match_date_is_in_past & match_not_processed_yet
...
def spring_classics() -> None:
    ...
    for idx, match in matches_to_scrape.iterrows():
        print(
            f"Processing match {idx+1} of {matches_to_scrape.shape[0]}: {match['MATCH']}."
        )

        results_data = scrape_website(results_data, match)
        results_data = calculate_match_points(results_data)
        results_data, message_data = calculate_match_standings(
            results_data, message_data
        )
        message_data = process_results.create_echelon_plot(
            results_data, message_data, gc=False
        )
    ...
    message_data.coach_mentions.append(
        process_results.list_best_coaches(results_data.all_points)
    )
    message_data = process_results.create_echelon_plot(
        results_data, message_data, gc=True
    )
...
def grand_tour():
    ...
    for idx, match in matches_to_scrape.iterrows():
        ...
        message_data = process_results.create_swarm_plot(
            results_data, message_data, gc_check=False
        )
