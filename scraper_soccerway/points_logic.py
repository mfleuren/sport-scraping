...
def calculate_point_by_player(data: CompetitionData) -> pd.DataFrame:
    ...
    points_player['P_Gewonnen'] = points_player['Wedstrijd_Gewonnen_p'] *\
             points_player['Wedstrijd_Gewonnen_e'] *\
             (points_player['Minuten_Gespeeld']/points_player['Match_Duration'])
    points_player['P_Gelijk'] = points_player['Wedstrijd_Gelijk_p'] *\
             points_player['Wedstrijd_Gelijk_e'] *\
             (points_player['Minuten_Gespeeld']/points_player['Match_Duration'])
    points_player['P_Verloren'] = points_player['Wedstrijd_Verloren_p'] *\
    ...
    points_player['P_Tegendoelpunt'] = points_player['Tegendoelpunt_p'] *\
             points_player['Tegendoelpunt_e'] *\
             (points_player['Minuten_Gespeeld'] > 0).astype('int')
    points_player['P_Eigen_Doelpunt'] = points_player['Eigen_Doelpunt'] *\
             points_player['Goal_Eigen']
    points_player['P_Clean_Sheet'] = points_player['CleanSheet_p'] *\
    ...
    cols_to_keep = ['Speler', 'Link', 'Datum'] + points_player.columns[points_player.columns.str.contains('P_')].tolist()
    ...
    points_player['P_Totaal'] = points_player.drop(['Speler', 'Link', 'Datum'], axis=1).sum(axis=1)
